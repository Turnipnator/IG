...
class _Base(unittest.TestCase):
    ...
    def _restricted(self, epic=GOLD):
        """A live-breakout config copy made long-only, to exercise the gate itself."""
        return dataclasses.replace(self._cfg(epic), allowed_direction="BUY")
...
class Helper(_Base):
    def test_wrong_way_on_restricted_market_is_blocked(self):
        self.assertTrue(main._breakout_direction_blocked(self._restricted(), self._sig(GOLD, "SELL")))
...
class HourClosePath(_Base):
    ...
    def _run(self, epic, direction, mode, name="Gold", cfg=None):
        ...
        with patch.object(main.breakout, "analyze_breakout", return_value=sig), \
            ...
            main.analyze_forex_breakout(epic, _Market(), cfg or self._cfg(epic), mode)
        return snap, execute

    def test_live_short_is_not_executed(self):
        snap, execute = self._run(GOLD, "SELL", "breakout", cfg=self._restricted())
        execute.assert_not_called()

    def test_live_short_is_still_snapshotted_for_the_counterfactual(self):
        snap, _ = self._run(GOLD, "SELL", "breakout", cfg=self._restricted())
    ...
    def test_live_long_still_executes(self):
        snap, execute = self._run(GOLD, "BUY", "breakout", cfg=self._restricted())
...
class OrderFunnelBackstop(_Base):
    ...
    def test_wrong_way_refused_before_any_broker_call(self):
        with patch.object(main, "client") as client, \
            ...
            main._execute_breakout_entry(GOLD, _Market(), self._restricted(),
...
class ConfigCoherence(_Base):
    ...
    def test_momentum_long_only_markets_keep_their_restriction(self):
        ...
        for epic in (SPX, "IX.D.FTSE.DAILY.IP"):
            self.assertEqual(self._cfg(epic).allowed_direction, "BUY", epic)
